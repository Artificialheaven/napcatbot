...
一个基于 DearPyGui 和 OneBot 协议的现代化 QQ 机器人框架，支持插件化扩展、图形化管理界面和自动断线重连。

## ✨ 特性

- 🎨 **图形化界面**：基于 DearPyGui 的美观 GUI，实时查看日志、监控状态
- 🔌 **插件系统**：简单的插件架构，轻松扩展功能
- 🔄 **自动重连**：WebSocket 断线自动重连，指数退避算法
- 📦 **依赖管理**：每个插件独立的依赖环境，避免冲突
- 📊 **实时监控**：消息统计、群列表、好友列表一目了然
- 🛡️ **稳定可靠**：完善的异常处理和日志记录
...
**作用**：配置 OneBot 连接信息、插件路径等。

**格式**：
```
{
    "show_heartbeat": false,    # 是否显示心跳日志
    "ws_address": "ws://127.0.0.1:3001",    # OneBot WebSocket 地址
    "token": "test",    # token
    "use_plugin_venv": true,    # 是否为每个插件创建独立的虚拟环境
    "max_reconnect_attempts": 0,    # 最大重连次数，0表示无限重连
    "reconnect_base_delay": 2,  # 重连初始延迟（秒）
...
class main:
    ...
    async def on_start(self):
        ...
        try:
            ...
            self.bot.Logger.info("框架", "任意값", "插件启动完成")
            self.bot.Logger.info("框架", "任意값", "早上好，" + self.nickname)
